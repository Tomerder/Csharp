...
namespace FinalizationPitfalls
{
    ...
    class Finalization
    {
        ...
        /// be finalized.  The Resource3 constructor takes 10ms but its finalizer
    ...
    class Resource3
    {
        ...
        public void Work()
        {
            ...
            // [Ofir]
            // This bug will only occure on Release mode (and Ctrl+F5) - duo to the Eager collection behaviour of the GC
            // In addition, couple of more intresting changes between debug in release mode:
            // 1. The RaceCondition() static method is being inlined into main (Use Debugger.Launch() in the Work method in release mode to show this - you'll notice that the RaceCondition() method is not in the call stack)
            // 2. The Use() method is also inlined, so we can't debug the If statement within the method
            Handle h = null;
            for (int i = 0; i < 10; i++)
            {
                h = _handle;
